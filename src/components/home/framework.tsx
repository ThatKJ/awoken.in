    ...
    <section className="py-16 md:py-20 lg:py-24 bg-surface">
      <Container>
        ...
        <div className="relative">
          ...
          {/* Mobile left line */}
          <div className="lg:hidden absolute left-5 top-0 bottom-0 w-px bg-border" />
          <div className="space-y-6 sm:space-y-8 lg:space-y-0 relative">
            {steps.map((step, i) => {
              ...
              return (
                ...
                >
                  <div className={`lg:w-1/2 ${isLeft ? "lg:pr-12 lg:text-right" : "lg:pl-12"}`}>
                    ...
                    <div className="lg:hidden pl-12">
                      <div className="inline-flex items-center gap-3 mb-3">
                        ...
                        <div>
                          <span className="text-xs sm:text-sm font-semibold text-accent">0{i + 1}</span>
                          <h3 className="text-base sm:text-xl font-bold">{step.title}</h3>
                        </div>
                      </div>
                      <ul className="space-y-1.5 sm:space-y-2">
                        {step.items.map((item) => (
                          <li key={item} className="text-sm sm:text-base text-muted-foreground flex items-center gap-2">
                    ...
                    <div className={`hidden lg:block ${isLeft ? "lg:pr-12 lg:text-right" : "lg:pl-12"}`}>
                  ...
                  <div className="lg:hidden absolute left-[14px] top-0 z-10 w-[22px] h-[22px] rounded-full border-2 border-accent bg-background flex items-center justify-center text-[10px] font-bold text-accent">
                    {i + 1}
                  </div>
